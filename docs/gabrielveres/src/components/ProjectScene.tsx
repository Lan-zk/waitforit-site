...
import projectManifest from "../../public/assets/manifest.json";
import type { ProjectTexture } from "@/types/project";

import styles from "./ProjectScene.module.css";

const projects: ProjectTexture[] = projectManifest;
...
function wrappedRelativeSlot(index: number, progress: number) {
  ...
  useEffect(() => {
    ...
    const syncTextureWindow = (progress: number) => {
      const candidates = projects
        .map((_, index) => ({
          index,
          relativeSlot: wrappedRelativeSlot(index, progress),
        }))
        .filter(
          ({ relativeSlot }) =>
    ...
    const updateMeshes = () => {
      ...
      meshes.forEach((mesh, index) => {
        const relativeSlot = wrappedRelativeSlot(index, progress);
        const visible =
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      window.location.assign(
        `https://www.gabrielveres.com${projects[clickedIndex].slug}`,
      );
    };
    ...
    <main className={styles.projectScene} aria-label="Selected projects">
      ...
      <nav className={styles.srOnly} aria-label="Project links">
        {projects.map((project) => (
          <a
            key={project.id}
            href={`https://www.gabrielveres.com${project.slug}`}
          >
            {project.title}
          </a>
        ))}
