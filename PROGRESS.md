...
- 确认简历使用现有 Lexical Global 还是结构化经历字段。
- [x] 为现有 Lexical `content` 建立官方 JSX 前台渲染器。
- [x] 处理空编辑器数据、畸形数据和中英文空状态。
- 保持作者内容原语言。
- 完成经历、技能、教育、联系方式和下载需求。
- 增加桌面、移动端和打印验证。

说明：按 2026-07-29 决定暂缓 M5。已经完成的 Lexical 前台渲染和双语空状态作为基础保留，不继续扩展 Schema、真实内容结构、下载或打印能力；恢复 M5 前再讨论“结构化字段”与“Markdown/富文本正文”的最终比例。
...
下一项工作仍是 **M4：自动同步的服务器验收**：

1. 在服务器构建或拉取 `content-sync` 工具镜像。
2. 配置 `/etc/waitforit/content-sync.env`、service 和 timer。
3. 验证相同 SHA 快速跳过，新 SHA 执行同步，失败后状态文件不推进且下一分钟重试。
4. 用新建、修改、删除测试文章验证运行中站点无需重新构建。
5. 验证完成前 M4 保持“进行中”，不得只凭本地配置将其标记为“已完成”。

**M5：简历已暂缓**。恢复前先与维护者讨论内容结构和建设路径，不默认在 M4 后立即进入。
